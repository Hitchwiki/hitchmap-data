...
class Transformer:
    ...
    # these are only used if the model provides standard deviations with predictions
    inverse_mean_func: None|Callable[[np.ndarray, np.ndarray], np.ndarray] = None
    inverse_std_func: None|Callable[[np.ndarray, np.ndarray], np.ndarray] = None
...
def get_transformer_with_least_skew(
        y: list[float|int|None] | pd.Series | np.ndarray) -> NumericTransformerOption:
    """
